...
pub const instance = struct {
    pub const UART0: UART = @as(UART, @enumFromInt(0));
    pub const UART1: UART = @as(UART, @enumFromInt(1));
...
pub const UART = enum(u1) {
    ...
    pub fn dma_dreq_tx(uart: UART) dma.Dreq {
        return switch (@intFromEnum(uart)) {
            0 => .uart0_tx,
            1 => .uart1_tx,
        };
    ...
    pub fn dma_dreq_rx(uart: UART) dma.Dreq {
        return switch (@intFromEnum(uart)) {
            0 => .uart0_rx,
            1 => .uart1_rx,
        };
    ...
    pub inline fn dma_tx_address(uart: UART) u32 {
        return @as(u32, @intFromPtr(&uart.get_regs().UARTDR));
    }

    pub const dma_rx_address = dma_tx_address;
